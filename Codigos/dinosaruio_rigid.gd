...
export var impulso = 10#variable que cambio desde el editor para determinar el impulso
var impulso_Vector = Vector3(0,impulso,0)#esto va a ser el impulso
var posicion = Vector3()#esto determina la posicion donde va a estar
var puede_saltar = false
var velocidad #determina la velocidad del dinosaurio

# Called when the node enters the scene tree for the first time.
func _ready():
	velocidad = linear_velocity.z #en un comienzo la velocidad va a ser 5

func _physics_process(delta):
	if linear_velocity.z < velocidad:#si la velocidad lineal es menor a la velocidad
		linear_velocity.z = velocidad#la velocidad lineal va a ser igual a la velocidad
	
	#var esta_en_colision = $Area.overlaps_area(get_tree().get_nodes_in_group("suelo_area")[0])
	posicion = translation#la posicion es la posición donde esta el dinosaurio
	if (Input.is_action_just_pressed("click_izquierdo") or Input.is_action_just_pressed("click_touch")) and puede_saltar:#si presiono click izquierdo
		...
		puede_saltar = false#no puede saltar hasta que esto sea verdero
		apply_impulse(posicion,impulso_Vector)#impulso hace que el personaje salte
